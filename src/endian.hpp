...
template <> inline u8 to_network_order(u8 val)
{
    return val;
}

template <> inline u16 to_network_order(u16 val)
{
...
template <> inline s16 to_network_order(s16 val)
{
    if (not is_little_endian()) {
        return val;
    } else {
...
template <> inline u32 to_network_order(u32 val)
{
    if (not is_little_endian()) {
        return val;
    } else {
...
template <> inline s32 to_network_order(s32 val)
{
    if (not is_little_endian()) {
        return val;
    } else {
...
template <> inline u64 to_network_order(u64 val)
{
    if (not is_little_endian()) {
        return val;
    } else {
...
template <> inline s64 to_network_order(s64 val)
{
    if (not is_little_endian()) {
        return val;
    } else {
...
template <typename T> class NetworkInteger {
public:
    // NOTE: I could overload the cast operator and assignment operators to make
    // this class behave similarly to a plain integer. But then I get
    // class-memaccess errors, which I don't want to disable throughout the
    ...
    explicit NetworkInteger(T value)
    {
        set(value);
    }
...
using network_u16 = NetworkInteger<u16>;
using network_u32 = NetworkInteger<u32>;
using network_s16 = NetworkInteger<s16>;
using network_s32 = NetworkInteger<s32>;
using network_u64 = NetworkInteger<u64>;
using network_s64 = NetworkInteger<s64>;
